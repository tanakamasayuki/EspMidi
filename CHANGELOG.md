...
- **`manual/` に「手動テスト」を追加した。** `manual/usb_if_din` は **USB MIDI インターフェースと MIDI DIN 回路を 1 本の輪**にして、`espmidi::UartPort` と `espmidi::UsbHostPort` を同じ router の中で相互に通す。**往路は DIN、復路は USB と物理層が別**なので、実物のフォトカプラと他社ファームウェアごと確認できる。手動なのは治具を組むところまでで、そのあとは自動で流れる。
- **手動テストは `test_` プレフィックスを付けない。** 機材が常時つながっていないので、`pytest` や `pytest manual/` では collect されず、ファイルを明示指定したときだけ動く。
- **GPIO はビルドに焼き込まない。** 回路につなぐピンは治具ごとに違うので、`.env` の `ESPMIDI_DIN_TX_PIN` / `ESPMIDI_DIN_RX_PIN` を pytest が読み、起動後にコンソールから `p <tx> <rx>` として渡す。arduino-cli の build property を触らずに済む。`.env.example` に雛形を追加した。
- `docs/LIBRARY_REQUESTS.ja.md` の依頼本文へのリンクを **GitHub の URL に直した**。相対パス(`../../EspUsbHost/...`)は隣に両リポジトリをチェックアウトしている環境でしか解決せず、CI で落ちていた。
- **`manual/usb_if_din` の待ちを経路指定にした。** どちらの経路から来ても待ちが解ける作りだったので、**IF が DIN IN を DIN OUT へ転送する治具では USB 経路が死んでいても先へ進んでしまった**(実機で発覚)。期待と違う経路から来たら `WRONG_PATH want=... got=...` を出す。
- **治具を組むための単発コマンドを `manual/usb_if_din` に足した。** `1`(DIN へ 1 通)/ `2`(USB へ 1 通)/ `r`(2 秒聞くだけ・生バイト)/ `t`(5 秒送り続ける・アナライザ用)/ `s`(状態)。**どれも実行したら戻る**ので、つなぎ替えもリセットも要らない。
- **`p` の再実行でルートが二重になっていた。** `addRoute()` は冪等ではないのに、席を供給する `attach*()` が冪等なので気付きにくい。**同じ 1 通が本数ぶん送られて本数ぶん配られる**(実機で 4 通ずつ返って発覚)。ルートのハンドルを持って、作り直す前に `removeRoute()` するようにした。
- **`manual/usb_if_din` に治具の事前判定を足した。** 本番の前に DIN へ 1 通出して、それが自分の MIDI IN に返ってきたら `RIG_FAULT the interface merges DIN IN into DIN OUT` と言って止まる。**MIDI Thru を持つ IF ではこの治具は成立しない**(往路と復路が区別できない)ことを、12 行の `WRONG_PATH` ではなく 1 行で伝える。
- **各受信行に送信からの経過時間を出す。** 中継と電気的な回り込みは時間で分かれる — 3 バイトは約 960 µs なので、**約 1000 µs なら回り込み、約 1900 µs 以上なら受け切ってからの送り直し**。実機では 1930 µs で、IF の Thru と確定した。**印字の順番は根拠にならない**(`pump()` が `adapter.update()` を先に呼ぶ)ことも文書に明記した。
- **`UartPort::begin()` が、開いていたシリアルを先に閉じるようにした。** ESP32 はペリフェラルを GPIO マトリクス経由でパッドにつなぐので、**2 つ目のパッドを開いても 1 つ目は外れない**。ピンを変えて `begin()` を呼び直すと**誰も指定していないピンに送信が出続ける**(実機の治具で `p 6 7` にしたのに GPIO9 から出ていて発覚)。`unit/uart_port` に固定し、`docs/PORTS` と `docs/PORT_AUTHORING` に「席は冪等でも、その下のペリフェラルは冪等ではない」ことを書いた。
- **ソフトリセットでは GPIO マトリクスが消えないことを治具の手順に書いた。** コアの帳簿(`uart_get_TxPin()`)は `-1` に戻るのに**前の実行で使ったピンが U1TXD を出し続ける**ので、指定していないピンから MIDI が出る。**「配線していない経路から信号が返る」**という形で切り分けを丸ごと狂わせた。ピンを変えるときは電源ごと落とす。`s` がコアの持っているピンを表示する。
- **`UartPort::begin()` の解放を、ピンが実際に変わったときだけにした。** 同じピンでの呼び直しは冪等な再設定なので、閉じて開き直すと受信バッファを捨て、線にグリッチを出す。**移動するときは `end()` と同じ順序で、送信中のダンプを `0xF7` で閉じてから**閉じる(規則 2)。`unit/uart_port` が 3 つの場合(移動・同一ピン・送信中の移動)を固定する。
- `loopback/uart_midi` が張る GPIO マトリクスの接続も**ソフトリセットで消えない**ことをスケッチと README に明記した。次のスケッチが指定していないピンから送信する。

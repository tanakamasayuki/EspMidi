...
- `ble_midi`: the BLE port boundary (Phase 8). The DUT is the host and the peer the device, over **a real radio link**: service discovery, **timestamps** (`unit=1` is `Milliseconds13`) and dumps each way. BLE MIDI does not pair, so no bonds are created. One test function per file.
...
Nothing yet: the bundled ports all have peer coverage.

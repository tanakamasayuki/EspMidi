// Peer side of the USB Host MIDI port test: EspMidi's USB Device port on an
// asymmetric multi-cable EspUsbDevice.
//
// The same device as tests/peer/usb_midi/peer_device. What differs is the other
// board: there it is watched by a plain EspUsbHost, here by EspMidi's own USB
// Host port. Running both means the device half was proven against an
// independent observer before it became one end of an EspMidi-to-EspMidi link.
//
...
//
// The DUT side reports what its USB Host port made of the same device. The two
// are mirror images — 3 in / 2 out here, 2 in / 3 out there — because the host
// view inverts for a device and does not for a host.
//

...
**この接続はスケッチより長く残ります。** GPIO マトリクスはレジスタで、**ソフトリセットでは消えません**。このテストのあと、Arduino のコアは「何も割り当てていない」(`uart_get_TxPin()` が `-1`)と思っているのに、**UART1 の送信信号が GPIO21 に残っている**という状態になり得ます。次のスケッチが**指定していないピンから送信します**。

**ピンを測る作業の前には、そのボードの電源を落としてください。** `manual/usb_if_din` の切り分けが半日狂った原因がこれです。
